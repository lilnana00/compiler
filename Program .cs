...
namespace Compiler
{
    class Program
    {
        static void Main(string[] args)
        {
                ...
                switch (args[0])
                {
                    case "-lexer-test":
                        ...
                        for (int i = 1; i <= testsCount; i++)
                        {
                            ...
                            string outputPath = string.Format(dir + @"\tests\lexer\output\{0}.txt", i);
                            string inputPath = string.Format(dir + @"\tests\lexer\input\{0}.txt", i);
                    ...
                    case "-se-test":
                        ...
                        {
                            string dir = Directory.GetParent(Environment.CurrentDirectory).Parent.Parent.FullName;
                            string outputPath = string.Format(dir + @"\tests\se\output\{0}.txt", i);
                            string inputPath = string.Format(dir + @"\tests\se\input\{0}.txt", i);
            ...
            /*else if (args.Length == 1 && args[0] == "-setestcreate")
            {
                ...
                for (int i = 1; i <= testsCount; i++)
                {
                    ...
                    FileStream output = File.OpenWrite(outputPath);
                    Lexer l = new Lexer(inputPath);
                    string answer = l.GetSimpleExpression();
